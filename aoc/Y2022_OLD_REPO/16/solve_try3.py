...
#Valve.assemble()
Valve.assemble_optimized(prunezero=False)

# ------------------------- ALGORITHMIQUE ----------------------

states = {valve.name:np.array([0]*State.total_time) for valve in Valve.valves}
s = State(states)
s.move_to_valve(Valve.valves_dict['DD'])
s.open_current_valve(0)
s.move_to_valve(Valve.valves_dict['AA'])
print(s.choose_action())
